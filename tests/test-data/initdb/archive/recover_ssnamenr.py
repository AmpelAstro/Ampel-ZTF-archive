...
def blobs_from_tarball(procnum, queue, date, partnership=True):
    ...
    try:
        if partnership:
            url = "https://ztf:16chipsOnPalomar@ztf.uw.edu/alerts/partnership/ztf_partnership_{}.tar.gz".format(
                date
            )
        else:
        ...
        if i > 0:
            log.info(
                "ztf_{}_{} finished ({} alerts)".format(
                    ["public", "partnership"][partnership], date, i
                )
            )
        queue.put(procnum)
...
class Updater:
    def __init__(self, connection, table, fields):
        ...
        self._query = (
            ...
            .where(
                pkey
                == select([pkey]).where(condition).with_for_update(skip_locked=True)
            )
            .values(**values)
        )
    ...
    def add(self, list_of_dicts):
        self._values += [
            {"b_" + k: item.get(k) for k in self._fields} for item in list_of_dicts
        ]
...
def ingest_blobs(procnum, queue, archive_url):
    ...
    update_candidate = Updater(
        db._connection,
        db._meta.tables["candidate"],
        ("isdiffpos", "ssnamenr", "magzpscirms"),
    )
    update_prv_candidate = Updater(
        db._connection, db._meta.tables["prv_candidate"], ("isdiffpos", "ssnamenr")
    )
    update_upper_limit = Updater(
        db._connection, db._meta.tables["upper_limit"], ("rbversion",)
    )
...
def recover(args):
    ...
    dates = [
        (begin + datetime.timedelta(i)).strftime("%Y%m%d")
        for i in range((datetime.datetime.now() - begin).days)
    ] * 2
    input_queue = multiprocessing.Queue(10 * args.workers)
    sources = {
        i: multiprocessing.Process(
            target=blobs_from_tarball, args=(i, input_queue, date, i % 2 == 0)
        )
    ...
    sinks = {
        i: multiprocessing.Process(
            target=ingest_blobs, args=(i, output_queues[i], args.archive)
        )
...
def check(args):
    ...
    jd = func.width_bucket(
        Alert.c.jd, Time(begin).jd, Time(end).jd, int(Time(end).jd - Time(begin).jd)
    )
    ...
    for date, bounds in zip(dates, bounds):
        for programid, name in programs.items():
            ...
            q = Alert.count(
                and_(
                    Alert.c.jd > bounds[0].jd,
                    Alert.c.jd < bounds[1].jd,
                    Alert.c.programid == programid,
                )
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--workers", type=int, default=4, help="Number of db clients to start"
    )
